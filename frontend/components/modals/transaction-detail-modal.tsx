...
import { confirmTransaction, revokeConfirmation, executeTransactionManual, isConfirmedBy } from '@/lib/web3';
...
export function TransactionDetailModal({
  ...
  const fetchSigners = async () => {
    if (!transaction || !controllerAddress || !owners.length) return;
    try {
      setLoadingSigners(true);
      const results = await Promise.all(
        owners.map(async (owner) => {
          try {
            const confirmed = await isConfirmedBy(controllerAddress, transaction.id, owner.address);
            return confirmed ? owner.address.toLowerCase() : null;
          } catch {
            return null;
          }
        })
      );
      setConfirmers(results.filter(Boolean) as string[]);
    } catch (e) {
  ...
  }, [isOpen, transaction?.id, controllerAddress]);
  ...
  const handleAction = async (action: 'confirm' | 'revoke' | 'execute') => {
    ...
    try {
      let txResponse;
      if (action === 'confirm') txResponse = await confirmTransaction(controllerAddress, transaction.id, DEFAULT_CHAIN_ID);
      if (action === 'revoke')  txResponse = await revokeConfirmation(controllerAddress, transaction.id, DEFAULT_CHAIN_ID);
      if (action === 'execute') txResponse = await executeTransactionManual(controllerAddress, transaction.id, DEFAULT_CHAIN_ID);
          ...
          <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <Badge
                  variant={transaction.executed ? 'default' : 'secondary'}
                  className={transaction.executed ? 'bg-emerald-500' : 'bg-orange-500/10 text-orange-500'}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-0.5">
                      <code className="font-mono bg-background px-1 py-0.5 rounded border">
                        {transaction.initiator.slice(0, 6)}...{transaction.initiator.slice(-4)}
                      </code>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 border rounded-lg bg-card shadow-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => handleCopy(transaction.to)}>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {confirmers.map((addr) => {
                      ...
                      return (
                        <div
                          key={addr}
                          className="flex items-center justify-between p-2.5 rounded-md bg-muted/40 border border-transparent hover:border-border transition-colors group"
                        >
