...
import { Wallet, Copy, Check, ExternalLink, ChevronDown, Power } from 'lucide-react';
import { cn } from '@/lib/utils';
import { getChainConfig } from '@/lib/chains';

const formatAddress = (addr: string, len = 4) =>
  `${addr.substring(0, len + 2)}...${addr.substring(addr.length - len)}`;

export function ConnectWallet() {
  const { address, chainId, isConnected, isConnecting, setShowModal, disconnect } = useWallet();
  ...
  if (isConnecting) {
    return (
      <div className="h-10 px-4 flex items-center gap-2 border-2 border-black/20 dark:border-white/20 bg-muted/10">
        <span className="text-xs font-bold uppercase tracking-widest opacity-50">Connecting...</span>
      </div>
    );
  }
        ...
        >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">{chainData.name}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between p-2 border-2 border-black dark:border-white bg-muted/10">
                <code className="text-xs font-mono font-bold truncate max-w-45">{address}</code>
                <button onClick={handleCopy} className="p-1 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" size="sm"
                ...
                onClick={() => window.open(`${chainData.explorer}/address/${address}`, '_blank')}
              >
                <ExternalLink className="h-3 w-3 mr-2" /> Explorer
              </Button>
